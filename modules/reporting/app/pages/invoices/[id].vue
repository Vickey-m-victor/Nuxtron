...
<template>
  <div class="content">
    <BasePageHeading :title="'View Invoices'">
      <template #extra>
        <button 
    ...
    <BaseBlock title="Invoices Details" content-full>
      <div v-if="pending" class="text-center py-5">
        <div class="spinner-border text-primary mb-3" role="status">
          <span class="visually-hidden">Loading...</span>
        </div>
        <p class="text-muted">Loading invoices details...</p>
      </div>
      ...
      <div v-else-if="error" class="alert alert-danger d-flex align-items-center" role="alert">
        ...
        <div class="flex-grow-1 ms-3">
          <p class="mb-0">{{ error.message || 'Failed to load invoices' }}</p>
        </div>
      </div>
      ...
      <div v-else-if="item" class="row g-4 push">
        ...
        <div class="col-md-6 col-xl-4">
          <div class="border-start border-primary border-3 ps-3">
            <p class="text-muted fs-xs fw-semibold text-uppercase mb-1">Date</p>
            <p v-if="typeof item.date === 'object' && item.date?.label && item.date?.theme" class="mb-0">
              <span :class="'fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-' + item.date.theme + '-light text-' + item.date.theme">{{ item.date.label }}</span>
            </p>
            <p v-else class="fs-sm fw-medium mb-0">{{ item.date || '-' }}</p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4">
          <div class="border-start border-primary border-3 ps-3">
            <p class="text-muted fs-xs fw-semibold text-uppercase mb-1">Supplier Id</p>
            <p v-if="typeof item.supplier_id === 'object' && item.supplier_id?.label && item.supplier_id?.theme" class="mb-0">
              <span :class="'fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-' + item.supplier_id.theme + '-light text-' + item.supplier_id.theme">{{ item.supplier_id.label }}</span>
            </p>
            <p v-else class="fs-sm fw-medium mb-0">{{ item.supplier_id || '-' }}</p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4">
          <div class="border-start border-primary border-3 ps-3">
            <p class="text-muted fs-xs fw-semibold text-uppercase mb-1">Invoice Amount</p>
            <p v-if="typeof item.invoice_amount === 'object' && item.invoice_amount?.label && item.invoice_amount?.theme" class="mb-0">
              <span :class="'fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-' + item.invoice_amount.theme + '-light text-' + item.invoice_amount.theme">{{ item.invoice_amount.label }}</span>
            </p>
            <p v-else class="fs-sm fw-medium mb-0">{{ item.invoice_amount || '-' }}</p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4">
          <div class="border-start border-primary border-3 ps-3">
            <p class="text-muted fs-xs fw-semibold text-uppercase mb-1">Payment Method Id</p>
            <p v-if="typeof item.payment_method_id === 'object' && item.payment_method_id?.label && item.payment_method_id?.theme" class="mb-0">
              <span :class="'fs-xs fw-semibold d-inline-block py-1 px-3 rounded-pill bg-' + item.payment_method_id.theme + '-light text-' + item.payment_method_id.theme">{{ item.payment_method_id.label }}</span>
            </p>
            <p v-else class="fs-sm fw-medium mb-0">{{ item.payment_method_id || '-' }}</p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4">
          <div class="border-start border-primary border-3 ps-3">

...
import type { SuppliersCreatePayload } from '../../types/suppliers-dto.js'
...
const onCreate = async () => {
  ...
  if (!modalStore.useModal) {
    ...
    router.push(`/reporting/suppliers/create`)
    return
  }

  // Modal mode: Define submit handler
  const handleSubmit = async (newData: SuppliersCreatePayload) => {
    try {
      const response = await $api(`/api/v1/reporting/supplier`, {
        method: 'POST',
        body: newData
      })
      ...
      if (response?.alertifyPayload) {
        ...
        showToast('Suppliers created successfully', 'success')
      }
...
// ========== VIEW HANDLER ==========
const onView = async (row: Suppliers) => {
  const id = row.id
  ...
  if (!modalStore.useModal) {
    ...
    router.push(`/reporting/suppliers/${id}`)
    return
  }
...
// ========== EDIT HANDLER ==========
const onEdit = async (row: Suppliers) => {
  const id = row.id
  errors.value = {}
  ...
  const handleSubmit = async (updatedData: Suppliers) => {
    try {
      const response = await $api(`/api/v1/reporting/supplier/${id}`, {
        method: 'PUT',
        body: updatedData
      })
...
const onDelete = async (row: Suppliers) => {
  ...
  if (result.isConfirmed) {
    try {
      const response = await $api(`/api/v1/reporting/supplier/${row.id}`, {
        method: 'DELETE'
      })
...
<template>
  <div class="content">
    <BasePageHeading :title="'Suppliers List'" />

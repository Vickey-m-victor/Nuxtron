...
import type { LoginCreatePayload } from '../types/login-dto.js'

interface Props {
  formData: LoginCreatePayload | any
    ...
    <div v-if="compact" class="compact-form">
      ...
      <div v-if="!hideSubmit" class="d-flex gap-2 pt-2">
        ...
        >
          ...
          {{ loading || isLoading ? 'Saving...' : 'Save Login' }}
        </button>
      </div>
    </div>
    ...
    <div v-else class="row push">
      <div class="col-lg-4">
        <p class="fs-sm text-muted">
          {{ readonly ? 'View login details' : 'Fill in the login information' }}
        </p>
      </div>

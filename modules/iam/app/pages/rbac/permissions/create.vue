...
const formData = ref<PermissionsCreatePayload>({
  name: '',
  description: '',
  ruleName: ''
})
...
const handleSubmit = async () => {
  ...
  try {
    const response = await $api(`/api/v1/iam/permissions`, {
      method: 'POST',
      body: formData.value
    })
...
<template>
  <div class="content">
    <BasePageHeading :title="'Create Permissions'">
      <template #extra>
        <button 
          type="button" 
    ...
    <BaseBlock title="New Permissions" subtitle="Fill in the details below" content-full>
      <!-- Error Alert -->
        ...
        <div class="row push">
          <div class="col-lg-4">
            <p class="fs-sm text-muted">
              Provide the required information to create a new permissions.
            </p>
          </div>

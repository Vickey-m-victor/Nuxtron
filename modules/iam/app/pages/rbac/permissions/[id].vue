...
<template>
  <div class="content">
    <BasePageHeading :title="'View Permissions'">
      <template #extra>
        <button 
    ...
    <BaseBlock title="Permissions Details" content-full>
      <div v-if="pending" class="text-center py-5">
        <div class="spinner-border text-primary mb-3" role="status">
          <span class="visually-hidden">Loading...</span>
        </div>
        <p class="text-muted">Loading permissions details...</p>
      </div>
      ...
      <div v-else-if="error" class="alert alert-danger d-flex align-items-center" role="alert">
        ...
        <div class="flex-grow-1 ms-3">
          <p class="mb-0">{{ error.message || 'Failed to load permissions' }}</p>
        </div>
      </div>

...
import type { GroupsCreatePayload } from '../../../types/groups-dto.js'
...
const onCreate = async () => {
  ...
  if (!modalStore.useModal) {
    ...
    router.push(`/iam/rbac/groups/create`)
    return
  }

  // Modal mode: Define submit handler
  const handleSubmit = async (newData: GroupsCreatePayload) => {
    try {
      const response = await $api(`/api/v1/iam/rbac/group`, {
        method: 'POST',
        body: newData
      })
      ...
      if (response?.alertifyPayload) {
        ...
        showToast('Groups created successfully', 'success')
      }
...
const onView = async (row: Groups) => {
  ...
  if (!modalStore.useModal) {
    ...
    router.push(`/iam/rbac/groups/${id}`)
    return
  }

  // Modal mode: fetch data and show in modal
  try {
    const response = await $api(`/api/v1/iam/rbac/group/${id}`)
...
const onEdit = async (row: Groups) => {
  ...
  try {
    ...
    const handleSubmit = async (updatedData: Groups) => {
      try {
        const response = await $api(`/api/v1/iam/rbac/group/${id}`, {
          method: 'PUT',
          body: updatedData
...
const onDelete = async (row: Groups) => {
  ...
  if (result.isConfirmed) {
    try {
      const response = await $api(`/api/v1/iam/rbac/group/${row.id}`, {
        method: 'DELETE'
      })
...
<template>
  <div class="content">
    <BasePageHeading :title="'Groups List'" />

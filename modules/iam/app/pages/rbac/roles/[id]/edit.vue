...
import type { Roles, RolesUpdatePayload } from '../../../types/roles-dto.js'
...
const { data } = await useFetch<{
  dataPayload: { data: Roles }
}>(`/api/v1/iam/rbac/role/${id}`, {
  $fetch: $api
})

const formData = ref<RolesUpdatePayload>({
  id: Number(id),
...
<template>
  <div class="content">
    <BasePageHeading :title="'Edit Roles'">
      <template #extra>
        <button 
          type="button" 
          class="btn btn-alt-secondary"
          @click="() => router.push(`/iam/rbac/roles`)"
        >
    ...
    <BaseBlock title="Update Roles" subtitle="Modify the details below" content-full>
      <!-- Error Alert -->
        ...
        <div class="row push">
          <div class="col-lg-4">
            <p class="fs-sm text-muted">
              Update the form fields to modify this roles.
            </p>
          </div>
